...
$(obj)u-boot-aml-ucl.bin: $(PLUS_BINARY) $(FIRMWARE) $(obj)u-boot-ucl.bin
	$(obj)tools/convert --soc $(SOC)  -s $(FIRMWARE) -i $(obj)u-boot-ucl.bin -o $(@:%=%.tmp)
ifneq ($(PLUS_BINARY),)	
...
#	$(MAKE) -C $(basename $(@F))/$(ARCH)/$(CPU)/$(SOC) all FIRMWARE=$@ UCL_BOOTLIBS=$(obj)lib/ucl/libucl.a
	...
	mv $(obj)usb_spl.bin $(obj)usb_spl.old.bin
	$(obj)tools/convert --soc $(SOC)  -s  $(obj)aml-debug-rom.old.bin -o $(obj)aml-debug-rom.bin
	$(obj)tools/convert --soc $(SOC)  -s  $(obj)usb_spl.old.bin -o $(obj)usb_spl.bin
...
$(VERSION_FILE):
		...
		@( printf '#define U_BOOT_PVERSION "%s"\n' \
		 '$(U_BOOT_VERSION)' ) >> $@.tmp
		@cmp -s $@ $@.tmp && rm -f $@.tmp || mv -f $@.tmp $@

$(TIMESTAMP_FILE):
		@LC_ALL=C date +'#define U_BOOT_DATE "%Y%m%d"' > $@
		@LC_ALL=C date +'#define U_BOOT_TIME "%H%M%S"' >> $@
...
#		$(MAKE) -C tools/env all MTD_VERSION=${MTD_VERSION}  || exit 1
		$(MAKE) -C tools/env all MTD_VERSION=${MTD_VERSION} CROSS_COMPILE=arm-none-linux-gnueabi- || exit 1
...
backup: distclean
	...
	tar --exclude=.git --force-local -jcvf `LC_ALL=C date "+$$F-%Y%m%d-%H%M%S.tar.bz2"` $$F
