...
static void load_mrs(int mrs_num, int mrs_value)
{
  APB_Wr(PCTL_MCMD_ADDR, (1 << 31) | 
                         (DDR_RANK << 20) |   //rank select
                   (mrs_num << 17) |
...
static void load_zqcl(int zqcl_value )
{
  APB_Wr(PCTL_MCMD_ADDR, (1 << 31) | 
                         (DDR_RANK << 20) |   //rank select
                  (zqcl_value << 4 ) |
...
static unsigned get_mrs0(struct ddr_set * timing_reg)
{
    ...
    //wr: write recovery 
     if(timing_reg->t_wr <= 8)
        ...
        ret|=((timing_reg->t_wr>>1)&0x7)<<9;
    return ret&0x1fff;
}

static unsigned get_mrs1(struct ddr_set * timing_reg)
{
    unsigned ret=0;
//    (0<<6)|(1<<2);//rtt_nominal;      //(A9, A6, A2) 000 : disabled. 001 : RZQ/4   010:RZQ/2 (A6:A2)
//    ret|=(0<<5)|(0<<1); //(A5 A1),Output driver impedance control 00:RZQ/6,01:RZQ/7,10£ºRZQ/5 11:Reserved
//    ret|=(0<<12)|(0<<7);
...
static unsigned get_mrs2(struct ddr_set * timing_reg)
{
...
#define ENABLE_POWER_SAVING
    ...
    APB_Wr(PCTL_PHYCR_ADDR, APB_Rd(PCTL_PHYCR_ADDR) & 0xfffffeff );   //disable auto data training when PCTL exit low power state.
      ...
      APB_Wr(PCTL_TRTW_ADDR,   timing_reg->t_rtw);
      ...
      APB_Wr(PCTL_ODTCFG_ADDR, 0x8);         //configure ODT
    //ZQ calibration 
    if ( 0 ) {
    APB_Wr(PCTL_ZQCR_ADDR, ( 1 << 31) | (0x7b <<16) );
        ...
        while ( APB_Rd(PCTL_ZQCR_ADDR ) & ( 1<<31) ) {}
         ...
         //APB_Wr(PCTL_ZQCR_ADDR, ( 1 << 24) | 0x32cc);
