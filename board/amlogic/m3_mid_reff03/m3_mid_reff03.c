...
static int  sdio_init(unsigned port,AML_CARD_SD_INFO_t *sdio)
#else
...
#ifdef AML_CARD_SD_INFO_DETAILED
static int  sdio_detect(unsigned port,AML_CARD_SD_INFO_t *sdio)
#else
...
#ifdef AML_CARD_SD_INFO_DETAILED
static void sdio_pwr_prepare(unsigned port,AML_CARD_SD_INFO_t *sdio)
#else
...
#ifdef AML_CARD_SD_INFO_DETAILED
static void sdio_pwr_on(unsigned port,AML_CARD_SD_INFO_t *sdio)
#else
...
#ifdef AML_CARD_SD_INFO_DETAILED
static void sdio_pwr_off(unsigned port,AML_CARD_SD_INFO_t *sdio)
#else
...
//	board_mmc_register(SDIO_PORT_XC_C);
	return 0;
}
...
//extern int before_main_end;
//int logo_display_end,logo_display_start,switch_boot_mode_start,switch_boot_mode_end,switch_boot_mode_mid;
...
int logo_display(void)
{
    ...
    //logo_display_end = get_utimer(0);
    //printf("call loop_main to finish logo display time:%d\n",logo_display_end-before_main_end);
    return ret;
    ...
    else
    {
        ...
        for(i = 0; i < SCAN_DEVICE_PARTITION; i++)
        {
            ...
            if(!run_command (str, 0))
            {
				...
				run_command ("mmc write 1 ${loadaddr} 0 60000", 0);
...
//	printf("ACT8942_REG1_ADDR: %x \n\n",pmu);
	...
	{
		if(!(is_ac_online_sh() || measure_capacity_battery_sh()))
		{
			power_down();
			printf("Power Down!\n");
			hang();
