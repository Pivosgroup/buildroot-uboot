...
int vcc2_power_off(void)
{
	int ret = 0;
	uint8_t val;
	axp_gpio_set_io(2,1);
	ret = axp_gpio_set_io(2, 0);
	if(ret)
	{
		return ret;
	}
    ...
    else
    {
        ...
        for(i = 0; i < SCAN_DEVICE_PARTITION; i++)
        {
            ...
            if(!run_command (str, 0))
            {
				...
				run_command ("sf erase 4000 60000", 0);
				run_command ("sf write 82000000 4000 60000", 0);
...
int vcc3_power_on(void)
{
	int ret = 0;
	uint8_t val;
	ret = axp_gpio_set_io(3, 1);
	if(ret)
	{
		return ret;
	}
	
	ret = axp_gpio_set_value(3, 0);
	if(ret)
	{
		return ret;
	}
	axp_read(0x95, &val);
	printf("%s: AXP20_GPIO3_CFG = %x\n", __FUNCTION__, val);
	return 0;
}

int vcc3_power_off(void)
{
	int ret = 0;
	uint8_t val;
	ret = axp_gpio_set_io(3, 0);
	if(ret)
	{
		return ret;
	}
...
inline int get_key()
{
    ...
    return(((adc_val >= 0x0) && (adc_val < 600)) ? 1 : 0);
}
		...
		switch(reboot_mode_current)
		{
			...
			default:
			{
				...
				else
				{
					...
					{
						printf("axp_charger_get_vbat: %d", axp_charger_get_vbat());
						if(axp_charger_get_vbat() < 3500)
						{						
							power_low_display();
							sdelay(2);
							printf("Low Power!!!\nPower Down!\n");
