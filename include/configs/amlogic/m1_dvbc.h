...
#endif
/*
When you choose multi logo solution, you need set these envs:
"aml_logo_start_720p=0x1000000\0" \
"aml_logo_start_1080p=0x1800000\0" \

change env as:
"normal_start=0x2000000\0" \

change recovery boot for nand to spi
nand:
    "recoveryboot=echo Enter Recovery mode ...;" \
        "nand read ${recovery_name} ${loadaddr} 0 ${recovery_size}; bootm ${loadaddr}\0" \
spi:
    "recoveryboot=echo Enter Recovery mode ...;" \
        "sf probe 2; sf read ${loadaddr} 40000 3c0000; bootm ${loadaddr}\0" \

Remove "ethaddr=00:1A:95:00:00:00\0" \ because it's repeated with CONFIG_ETHADDR.
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	" logo=osd1,${loadlogoaddr},full,${outputmode},loaded androidboot.resolution=${outputmode}" \
    " resolution_size=${screen_width},${screen_height}" \
	" mem=${memsize} a9_clk=${cpuclock} clk81=${gpuclock} mac=${ethaddr}\0" \
    "nandboot=echo Booting from nand ...; " \
        "nand read ${aml_logo_name} ${loadlogoaddr} 0 ${aml_logo_size}; "\
